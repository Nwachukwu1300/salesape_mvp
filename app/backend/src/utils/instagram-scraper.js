 ...
 */
export function extractContactFromBio(bio) {
    const contacts = {};
    // Email regex
    const emailMatch = bio.match(/([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]+)/);
    if (emailMatch?.[1])
        contacts.email = emailMatch[1];
    ...
    if (websiteMatch?.[1])
        contacts.website = websiteMatch[1];
    return contacts;
}
/**
 * Convert Instagram profile to business data
 */
export function instagramProfileToBusinessData(profile) {
    return {
 ...
 */
export function generateInstagramHashtags(businessType) {
    const hashtagSets = {
        landscaping: [
            '#landscaping',
            '#gardendesign',
    ...
    const type = Object.keys(hashtagSets).find(key => businessType.toLowerCase().includes(key));
    return hashtagSets[type] || hashtagSets.services;
}
/**
 * Generate Instagram post caption template
 */
export function generateInstagramCaption(businessName, service = 'services', cta = 'DM for more information') {
