 ...
 */
export function determineWinner(test, confidenceThreshold = 95) {
    const rateA = calculateConversionRate(test.metrics.variant_a.conversions, test.metrics.variant_a.clicks);
    const rateB = calculateConversionRate(test.metrics.variant_b.conversions, test.metrics.variant_b.clicks);
    const confidence = calculateStatisticalSignificance({ conversions: test.metrics.variant_a.conversions, views: test.metrics.variant_a.views }, { conversions: test.metrics.variant_b.conversions, views: test.metrics.variant_b.views });
 ...
 */
export function generateTestRecommendations(test) {
 ...
 */
export function calculateSampleSize(baselineConversionRate, minimalDetectableEffect = 0.2, // 20% relative improvement
confidenceLevel = 0.95, statisticalPower = 0.8) {
    ...
    const denominator = ((p2 - p1) * (p2 - p1)) / (p1 * (1 - p1) + p2 * (1 - p2));
