...
import { ArrowLeft, ArrowRight, Edit2, Check, AlertCircle } from "lucide-react";
...
export function RecapScreen() {
  ...
  const businessData = (
    location.state as { businessUnderstanding?: BusinessUnderstanding }
  ...
  const handleFieldChange = (
    field: keyof BusinessUnderstanding,
    value: any,
  ) => {
    setData({
      ...data,
  ...
  const handleArrayFieldChange = (
    field: "services" | "trustSignals" | "seoKeywords" | "brandColors",
    index: number,
    value: string,
  ) => {
  ...
  const handleRemoveArrayItem = (
    ...
    const updatedArray = (data[field] as string[]).filter(
      (_, i) => i !== index,
    );
    handleFieldChange(field, updatedArray);
  };
  ...
  const validateData = () => {
    ...
    if (!data.valueProposition || data.valueProposition.trim().length < 10)
      newErrors.push("Value proposition must be at least 10 characters");
    if (!data.targetAudience || data.targetAudience.trim().length === 0)
      newErrors.push("Target audience is required");
    if (!data.trustSignals || data.trustSignals.length === 0)
      newErrors.push("At least one trust signal is required");
    if (!data.seoKeywords || data.seoKeywords.length < 5)
      newErrors.push("At least 5 SEO keywords are required");
        ...
        <Card className="max-w-md">
          <CardContent className="pt-6">
            ...
            <p className="text-slate-600 dark:text-slate-400 mb-6">
              Unable to load business data. Please start from the onboarding
              flow.
            </p>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleFieldChange("brandTone", e.target.value)
                  }
                  disabled={!isEditing}
            ...
            {(data.services || []).map((service, idx) => (
              <div key={idx} className="flex gap-2 items-end">
                <Input
                  value={service}
                  onChange={(e) =>
                    handleArrayFieldChange("services", idx, e.target.value)
                  }
                  disabled={!isEditing}
            ...
            {(data.trustSignals || []).map((signal, idx) => (
              <div key={idx} className="flex gap-2 items-end">
                <Input
                  value={signal}
                  onChange={(e) =>
                    handleArrayFieldChange("trustSignals", idx, e.target.value)
                  }
                  disabled={!isEditing}
              ...
              {(data.seoKeywords || []).map((keyword, idx) => (
                <div key={idx} className="flex gap-2">
                  <Input
                    value={keyword}
                    onChange={(e) =>
                      handleArrayFieldChange("seoKeywords", idx, e.target.value)
                    }
                    disabled={!isEditing}
                    placeholder="Keyword"
                    className="flex-1"
              ...
              {(data.brandColors || []).map((color, idx) => (
                <div key={idx} className="flex gap-2 items-end">
                  <Input
                    ...
                    value={color}
                    onChange={(e) =>
                      handleArrayFieldChange("brandColors", idx, e.target.value)
                    }
                    disabled={!isEditing}
                    placeholder="#FF0000"
                    className="flex-1"
