...
export function SubscriptionProvider({ children }: { children: ReactNode }) {
  ...
  const fetchSubscription = async () => {
    ...
    try {
      ...
      const firstBiz =
        Array.isArray(bizList) && bizList.length > 0 ? bizList[0] : null;
      ...
      const subRes = await fetch(
        `${API_BASE}/businesses/${firstBiz.id}/subscription`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      ...
      const planKey =
        (subscription && subscription.planId && planMap[subscription.planId]) ||
        "free";
  ...
  const fetchUsage = async () => {
    try {
      ...
      if (firstBiz && firstBiz.id) {
        try {
          const analyticsRes = await fetch(
            `${API_BASE}/businesses/${firstBiz.id}/analytics`,
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          );
      ...
      const leads =
        analytics?.summary?.totalLeads ?? analytics?.summary?.leadsCreated ?? 0;
      ...
      if (firstBiz && firstBiz.id) {
        try {
          const usageRes = await fetch(
            `${API_BASE}/businesses/${firstBiz.id}/usage`,
            {
              headers: { Authorization: `Bearer ${token}` },
            },
          );
